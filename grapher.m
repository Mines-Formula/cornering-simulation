...
for i = 1:length(uniqueLoads)
    ...
    % Pacejka Stuff
    x = deg2rad(binnedSlipAngle);
    y = loessForce;

    pacejkaFunction = @(p, x) p(3) .* sin(p(2) .* atan(p(1).*x - p(4).*(p(1).*x - atan(p(1).*x))));

    p0 = [10, 1.3, max(abs(y)), 0.97];
    lb = [0, 0.5, 0, 0];
    ub = [100, 3, 2 * max(abs(y)), 1];

    opts = optimoptions('lsqcurvefit', 'Display', 'off');
    [pFit, ~, residual] = lsqcurvefit(pacejkaFunction, p0, x, y, lb, ub, opts);

    SSResult = sum(residual.^2);
    SSTotal = sum((y - mean(y)).^2);
    R2 = 1 - SSResult/SSTotal;

    xFit = linspace(min(x), max(x), 400);
    yFit = pacejkaFunction(pFit, xFit);

    plot(rad2deg(xFit), yFit, '-', 'LineWidth', 2.5, 'Color', colors(i,:), 'DisplayName', sprintf('%d N (Pacejka)', thisLoad));

    fprintf('\n--- %.0f N Load ---\n', thisLoad);
    fprintf('B = %.4f, C = %.4f, D = %.4f, E = %.4f, R^2 = %.4f\n', pFit(1), pFit(2), pFit(3), pFit(4), R2);
    
    results = [results; {thisLoad, pFit(1), pFit(2), pFit(3), pFit(4), R2}];
...
fprintf("\n Pacejka parameter table saved as pacejka_fits.csv\n");
disp(results);